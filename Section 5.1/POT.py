...
def sample_from_Fz(ksi_z, nrsamp_perz):
    ...
    if ksi_z_0.shape[0]>0:
        x=np.concatenate((np.random.rand(ksi_z_pos.shape[0])**(-ksi_z_pos),(np.random.rand(ksi_z_neg.shape[0])**(-ksi_z_neg)-1)/ksi_z_neg,-np.log(ksi_z_0.shape[0])))
    else:
        x=np.concatenate((np.random.rand(ksi_z_pos.shape[0])**(-ksi_z_pos),(np.random.rand(ksi_z_neg.shape[0])**(-ksi_z_neg)-1)/ksi_z_neg))
    return x.squeeze()
