...
def dedh_estimator(x,thresh,avg_size):
    ...
    for j in np.arange(depth)[-avg_size:]:
        k=int(j+1)
        h1=np.append(h1,(np.log(x)[:k]-np.log(x)[k+1]).mean(0))
        h2=np.append(h2,((np.log(x)[:k]-np.log(x)[k+1])**2).mean(0))
...
degrees=(1.025**np.arange(1200)).astype(int)[10:870:20]
...
#We import 10000 windows of data, where each window has size 3. The first 2 values are used in order to predict the 3rd value.
...
for degree in degrees:
    ...
    for i in range(cross_iterations):
        ...
        #Fit X_T(i)
        kernel = 1.0 * RBF(degree, length_scale_bounds="fixed")
        gpr = GaussianProcessRegressor(kernel=kernel, random_state=0)
        gpr.fit(data_train[:,:-1], data_train[:,-1])

        #Predict X\X_T(i) and append to arr
        arr=np.append(arr,gpr.predict(data_test[:,:-1])[:-4])
    ...
    predictions=arr.copy().reshape(cross_iterations,-1)
    arr_loc=np.abs(arr.copy().reshape(cross_iterations,-1).T)
    ...
    arr=np.abs(arr.reshape(5*cross_iterations,-1)).T
    #Calculate the shape parameter of the tail for each individual distribution
    ksi_var=dedh_estimator(arr,0.97,5)
    ksi_var=ksi_var.reshape(-1,5).mean(1)
        


    cutoff=int(0.97*arr_loc.shape[0])
    print(arr_loc.shape)
    print('cutoff: ', arr_loc.shape[0])
    large_samps=-np.sort(-np.sort(arr_loc, axis=0)[cutoff:], axis=0)
    print(large_samps)
    print(large_samps.min(0).squeeze())
    np.savetxt('./var_loc/'+'run_'+run+'deg_'+str(degree)+'.txt',np.concatenate((ksi_var.squeeze(),large_samps.min(0).squeeze()),0))
    ...
    totksi_var=dedh_estimator(arr2.reshape(-1,5),0.997,5)

    #Save it
    rez.append(totksi_var.mean())
...
#np.savetxt('gaussprocess_CTE_potavg/'+'run_'+run+'.txt',rezmat)
