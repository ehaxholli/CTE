...
for degree in degrees:
    ...
    for i in range(training_sample_nr):
        if i%20==0:
            print('i:', i )
        ...
        clf= SVR( kernel='poly',gamma='scale', degree=degree)
        clf.fit(data2[:samp_size//2,:-1], data2[:samp_size//2,-1])

        #Predict on the entire X and append to arr
        arr=np.append(arr,clf.predict(data[:,:-1]))

        #calculate the predicted values in order to calculate the test MSE on data2.
        mse_train.append(((clf.predict(data2[samp_size//2:,:-1])-data2[samp_size//2:,-1])**2).mean())
    ...
    predictions=arr.copy().reshape(training_sample_nr,data.shape[0])
...
np.savetxt('PolyResultsT/'+'run_'+run+'.txt',rezmat)
