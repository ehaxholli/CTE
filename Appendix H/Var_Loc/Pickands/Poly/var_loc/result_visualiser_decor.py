...
allresults = []
plot_count = 0
fig, axs = plt.subplots(b, a, figsize=(a*5, b*5))
for filename in natsorted(os.listdir(os.getcwd())):
    if filename[:3] == 'run':
        ...
        axs[plot_count // a, plot_count % a].plot(ksis[sort_order])
        axs[plot_count // a, plot_count % a].set_title(plot_title, fontsize=14)
        axs[plot_count // a, plot_count % a].tick_params(axis='both', which='major', labelsize=14)

        plot_title = 'Scale: '+scale_list[0]+r'. Threshold'
        plot_count += 1
        axs[plot_count // a, plot_count % a].plot(locs[sort_order])
        axs[plot_count // a, plot_count % a].set_title(plot_title, fontsize=14)
        axs[plot_count // a, plot_count % a].tick_params(axis='both', which='major', labelsize=14)
